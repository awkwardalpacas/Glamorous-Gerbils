...
app.get('/wait', function (req, res) {
  ...
  db.getAllRestaurants(function(err, results){
    if (err){
      console.log(err);
    } else {
      var data = [];
      ...
      for (var i=0; i< allRestaurants.length; i++){
        var marker = {};
        var id = allRestaurants[i].google_id;
        console.log("This is id:", id);
        marker["google_id"] = id;

        db.getLatestAvgWaitAtLocation(id, function(results){
          console.log("This is wait results:", results);
          marker["wait"] = results;
        });

        db.getLatestReportTimestampById(id, function(error, results){
          console.log("This is timestamp results:", results);
          marker["timestamp"] = results;
          data.push(marker);
        });
      }
